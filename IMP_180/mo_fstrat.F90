    ...
    table_loop :  do i = 1,ub_nspecies
       ...
       if( map(i) == 0 ) then
          if( trim(ub_species_names(i)) == 'OX' ) then
             ...
             if( map(i) == 0 ) then
!PS                write(iulog,*) 'fstrat_inti: ubc table species ',trim(ub_species_names(i)), ' not used'
             end if
...
!PS          write(iulog,*) 'fstrat_inti: ubc table species ',trim(ub_species_names(table_nox_ndx)), ' not used'
       endif
    end if
...
!PS          write(iulog,*) 'fstrat_inti: ubc table species ',trim(ub_species_names(table_h2o_ndx)), ' not used'
       endif
    end if
