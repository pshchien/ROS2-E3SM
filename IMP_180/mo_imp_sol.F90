...
!PS
    do i = 1,nzcnt
      do j = 1,itermax
        write(fld_str,'(A,I3.3,A,I2.2)') 'MAT_',i,'_',j
        call addfld( trim(fld_str), (/ 'lev' /), 'I', 'dimless', 'matrix index' )
      enddo
    enddo
    do i = 1,clscnt4
      do j = 1,itermax
        write(fld_str,'(A,I3.3,A,I2.2)') 'FORC_',i,'_',j
        call addfld( trim(fld_str), (/ 'lev' /), 'I', 'NaN', 'forcing term' )
        write(fld_str,'(A,I3.3,A,I2.2)') 'SOL_',i,'_',j
        call addfld( trim(fld_str), (/ 'lev' /), 'I', 'NaN', 'solution term' )
      enddo
    enddo
...
!PS
    real(r8), dimension(ncol,pver,max(1,nzcnt),itermax) :: mat_out
    real(r8), dimension(ncol,pver,max(1,gas_pcnst),itermax) :: lsol_out, for_out
    character(LEN=20) :: fld_str
    mat_out(:,:,:,:) = 0._r8
    lsol_out(:,:,:,:) = 0._r8
    for_out(:,:,:,:) = 0._r8
...
!PS
                do m = 1,nzcnt
                  mat_out(i,lev,m,nr_iter) = sys_jac(m)
                enddo
                do m = 1,clscnt4
                  for_out(i,lev,m,nr_iter) = forcing(m)
                enddo
                call lu_slv( sys_jac, forcing )
!PS
                do m = 1,clscnt4
                  lsol_out(i,lev,m,nr_iter) = forcing(m)
                enddo
...
!PS
    ...
    do i = 1,nzcnt
      do j = 1,itermax
        write(fld_str,'(A,I3.3,A,I2.2)') 'MAT_',i,'_',j
        call outfld( trim(fld_str), mat_out(:,:,i,j), ncol, lchnk )
      enddo
    enddo
    do i = 1,clscnt4
      do j = 1,itermax
        write(fld_str,'(A,I3.3,A,I2.2)') 'FORC_',i,'_',j
        call outfld( trim(fld_str), for_out(:,:,i,j), ncol, lchnk )
        write(fld_str,'(A,I3.3,A,I2.2)') 'SOL_',i,'_',j
        call outfld( trim(fld_str), lsol_out(:,:,i,j), ncol, lchnk )
      enddo
    enddo
