...
module mo_imp_sol
  ...
  !-----------------------------------------------------------------------
  !PSinteger, parameter :: itermax = 11
...
!PS
    ...
    integer :: nstep
!PS
    integer :: nstep_size, num_thread
    integer :: omp_get_thread_num, num_cnt
...
!PS
    num_cnt = 0
    nstep_size = get_step_size()
!PS    call cpu_time(start)
    call t_startf("UCI_CHEM")
    level_loop : do lev = 1,pver
       column_loop : do i = 1,ncol
          ...
          time_step_loop : do
             ...
             if( cls_rxt_cnt(1,4) > 0 .or. extcnt > 0 ) then
                do m = 1,clscnt4
                   !PSiter_invariant(m) = dti * solution(m) + ind_prd(i,lev,m)
                   iter_invariant(m) = ind_prd(i,lev,m)
                end do
             else
                do m = 1,clscnt4
                   !PSiter_invariant(m) = dti * solution(m)
             ...
             iter_loop : do nr_iter = 1,itermax
                ...
                do m = 1,clscnt4
                   !PSforcing(m) = solution(m)*dti - (iter_invariant(m) + prod(m) - loss(m))
                   forcing(m) = (iter_invariant(m) + prod(m) - loss(m))
                end do
                !-----------------------------------------------------------------------
                ! ... solve for k1 !PS the mixing ratio at t(n+1)
                !-----------------------------------------------------------------------
...
!PS
!PS    nstep = get_nstep()
!PS    num_thread = omp_get_thread_num()
!PS    write(iulog,*) 'ha ha, pshchien, You are now in nstep= ', nstep, nstep_size, dt, delt, num_thread, &
!PS      cut_cnt, num_cnt, pver, ncol, nr_iter
!PS
