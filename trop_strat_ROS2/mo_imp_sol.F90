...
!PS  use chem_mods,    only : clscnt4, gas_pcnst, clsmap
  ...
  !-----------------------------------------------------------------------
!PS  integer, parameter :: itermax = 11
...
!PS
!PS    do i = 1,nzcnt
!PS      write(fld_str,'(A,I4.4)') 'MAT_', i
!PS      call addfld( trim(fld_str), (/ 'lev' /), 'I', 'dimless', 'matrix index' )
!PS    enddo
    do i=1,clscnt4
      write(fld_str,'(A,I4.4)') 'FORC_',i
      call addfld( trim(fld_str), (/ 'lev' /), 'I', 'dimless', 'forcing term' )
      write(fld_str,'(A,I4.4)') 'SOL_',i
      call addfld( trim(fld_str), (/ 'lev' /), 'I', 'dimless', 'solution term' ) 
    enddo
  ...
  subroutine imp_sol( base_sol, reaction_rates, het_rates, extfrc, delt, &
    ...
    use mo_indprd, only : indprd, indprd_box
...
!PS
    ...
    integer :: nr_iter, &
         ...
         m
!PS
    integer :: omp_get_num_threads, number_p
    integer :: fail_cnt, cut_cnt, stp_con_cnt
    integer :: nstep
    real(r8) :: interval_done, dt, dti, wrk, start, finish
...
!PS
    real(r8), dimension(ncol,pver,max(1,nzcnt),11) :: mat_out
    real(r8), dimension(ncol,pver,max(1,gas_pcnst),11) :: lsol_out, for_out
    character(LEN=20) :: fld_str
    mat_out(:,:,:,:) = 0._r8
    lsol_out(:,:,:,:) = 0._r8
    for_out(:,:,:,:) = 0._r8
...
!PS
!PS    call cpu_time(start)
    call t_startf("TROP_START_ONE")
!PS
!PS    number_p=omp_get_num_threads()
!PS    write(iulog,*) 'This is for checking number of threads',number_p
...
!PS
          ...
          time_step_loop : do
             ...
             if( cls_rxt_cnt(1,4) > 0 .or. extcnt > 0 ) then
                do m = 1,clscnt4
!PS                   iter_invariant(m) = dti * solution(m) + ind_prd(i,lev,m)
                   iter_invariant(m) = ind_prd(i,lev,m)
                end do
             else
                do m = 1,clscnt4
!PS                   iter_invariant(m) = dti * solution(m)
                   iter_invariant(m) = 0._r8
                end do
             end if
             !-----------------------------------------------------------------------
             ...
             iter_loop : do nr_iter = 1,itermax
                ...
                !-----------------------------------------------------------------------
!PS                if( factor(nr_iter) ) then
!PS                   call nlnmat( sys_jac, lsol, lrxt, lin_jac, dti )
                   call nlnmat( sys_jac, lsol, lrxt, lin_jac, dt )
                   !-----------------------------------------------------------------------
...
!PS                end if
                ...
                do m = 1,clscnt4
!PS                   forcing(m) = solution(m)*dti - (iter_invariant(m) + prod(m) - loss(m))
                   forcing(m) = (iter_invariant(m) + prod(m) - loss(m))
                end do
                !-----------------------------------------------------------------------
                ...
                do m = 1,clscnt4
                   solution(m) = solution(m) + forcing(m) *dt
                end do
                do m = 1, clscnt4
                   j = clsmap(m, 4)
                   k = permute(m, 4)
                   if (solution(k) < 0._r8) then
                      solution(k) = 0._r8
                   end if
                   lsol(j) = solution(k)
!PS                   base_sol(i,lev,j) = lsol(j)
                end do

                if( cls_rxt_cnt(1,4) > 0 .or. extcnt > 0 ) then
                    call indprd_box( ind_prd(i,lev,:), clscnt4, lsol, extfrc(i,lev,:), &
                         reaction_rates(i,lev,:) )
                else
                    do m = 1,max(1,clscnt4)
                       ind_prd(i,lev,m) = 0._r8
                    end do
                end if

                if( cls_rxt_cnt(1,4) > 0 .or. extcnt > 0 ) then
                    do m = 1,clscnt4
                       if (ind_prd(i,lev,m) < 0._r8) then
                          iter_invariant(m) = 0._r8
                       else
                          iter_invariant(m) = ind_prd(i,lev,m)
                       end if
                ...
                call imp_prod_loss( prod_temp, loss_temp, lsol, lrxt, lhet )
                do m = 1,clscnt4
                   forcing_temp(m) = (iter_invariant(m) + prod_temp(m) - loss_temp(m)) - 2._r8*forcing(m)
                end do
                !-----------------------------------------------------------------------
                ! ... solve the k2
                !-----------------------------------------------------------------------
!PS
                do m = 1,nzcnt
                  mat_out(i,lev,m,nstep) = sys_jac(m)
                end do
                do m = 1,clscnt4
                  for_out(i,lev,m,nstep) = forcing_temp(m)
                enddo
                call lu_slv( sys_jac, forcing_temp)
                do m = 1,clscnt4
                  lsol_out(i,lev,m,nstep) = forcing_temp(m)
                enddo
                !-----------------------------------------------------------------------
                ! ... Solve for mixing ratio at t(n+1)
                !-----------------------------------------------------------------------
                do m = 1,clscnt4
                   solution(m) = solution(m) + 0.5_r8*forcing(m)*dt + 0.5_r8*forcing_temp(m)*dt
                end do
!PS
                !-----------------------------------------------------------------------
                ! ... convergence measures
                !-----------------------------------------------------------------------
!PS                if( nr_iter > 1 ) then
!PS                   do k = 1,clscnt4
!PS                      m = permute(k,4)
!PS                      if( abs(solution(m)) > 1.e-20_r8 ) then
!PS                         max_delta(k) = abs( forcing(m)/solution(m) )
!PS                      else
!PS                         max_delta(k) = 0._r8
!PS                      end if
!PS                   end do
!PS                end if
                !-----------------------------------------------------------------------
...
!PS                   do k = 1,clscnt4
!PS                      m = permute(k,4)
!PS                      frc_mask = abs( forcing(m) ) > small
!PS                      if( frc_mask ) then
!PS                         converged(k) = abs(forcing(m)) <= epsilon(k)*abs(solution(m))
!PS                      else
!PS                         converged(k) = .true.
!PS                      end if
!PS                   end do
!PS                   convergence = all( converged(:) )
!PS                   if( convergence ) then
!PS                      exit
!PS                   end if
!PS                end if
             end do iter_loop
             ...
             !-----------------------------------------------------------------------
!PS             if( .not. convergence ) then
                !-----------------------------------------------------------------------
                ! ... non-convergence
                !-----------------------------------------------------------------------
!PS                fail_cnt = fail_cnt + 1
!PS                nstep = get_nstep()
!PS                write(iulog,'('' imp_sol: Time step '',1p,e21.13,'' failed to converge @ (lchnk,lev,col,nstep) = '',4i6)') &
!PS                     dt,lchnk,lev,i,nstep
!PS                stp_con_cnt = 0
!PS                if( cut_cnt < cut_limit ) then
!PS                   cut_cnt = cut_cnt + 1
!PS                   if( cut_cnt < cut_limit ) then
!PS                      dt = .5_r8 * dt
!PS                   else
!PS                      dt = .1_r8 * dt
!PS                   end if
!PS                   cycle time_step_loop
!PS                else
!PS                   write(iulog,'('' imp_sol: Failed to converge @ (lchnk,lev,col,nstep,dt,time) = '',4i6,1p,2e21.13)') &
!PS                        lchnk,lev,i,nstep,dt,interval_done+dt
!PS                   do m = 1,clscnt4
!PS                      if( .not. converged(m) ) then
!PS                         write(iulog,'(1x,a8,1x,1pe10.3)') solsym(clsmap(m,4)), max_delta(m)
!PS                      end if
!PS                   end do
!PS                end if
!PS             end if
...
!PS                if( fail_cnt > 0 ) then
!PS                   write(iulog,*) 'imp_sol : @ (lchnk,lev,col) = ',lchnk,lev,i,' failed ',fail_cnt,' times'
!PS                end if
                exit time_step_loop
             else
                !-----------------------------------------------------------------------
                ...
                !-----------------------------------------------------------------------
!PS                if( convergence ) then
!PS                   stp_con_cnt = stp_con_cnt + 1
!PS                end if
                do m = 1,gas_pcnst
                   base_sol(i,lev,m) = lsol(m)
                end do
!PS                if( stp_con_cnt >= 2 ) then
!PS                   dt = 2._r8*dt
!PS                   stp_con_cnt = 0
!PS                end if
!PS                dt = min( dt,delt-interval_done )
    ...
    call t_stopf("TROP_START_ONE")
!PS
!PS    call cpu_time(finish)
!PS    write(iulog,'(A,e21.3)') 'Time Checking with cpu_time = ', finish-start
    do i = 1,clscnt4
       j = clsmap(i,4)
...
!PS    do i = 1,nzcnt
!PS      write(fld_str,'(A,I4.4)') 'MAT_', i
!PS      write(iulog,'(A,A)') 'Write out ', trim(fld_str)
!PS      call outfld( trim(fld_str), mat_out(:,:,i,10), ncol, lchnk )
!PS      call outfld( trim(fld_str), mat_out(:,:,i), ncol, lchnk )
!PS    enddo
    do i = 1,clscnt4
      write(fld_str,'(A,I4.4)') 'FORC_',i
      call outfld( trim(fld_str), for_out(:,:,i,10), ncol, lchnk )
      write(fld_str,'(A,I4.4)') 'SOL_',i
      call outfld( trim(fld_str), lsol_out(:,:,i,10), ncol, lchnk )
    enddo
